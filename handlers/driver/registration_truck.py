...
class RegistrationDriverTruck(StatesGroup):
    ...
    async def menu_phone(self, message: Union[types.Message, types.CallbackQuery], state: FSMContext):
        ...
        async with state.proxy() as data:
            ...
            await bot.send_message(chat_id=message.from_user.id, text=await form.agreement_truck(),
    ...
    def register_handlers(self, dp: Dispatcher):
        dp.register_callback_query_handler(self.menu_car_type, text="truck",                                            state="RegistrationDriverCar:registration_level2")
        dp.register_callback_query_handler(self.menu_car_type, text="back",                                             state=self.registration_level2)

        dp.register_callback_query_handler(self.menu_car_subtype, lambda x: x.data.startswith("subtype"),               state=self.registration_level1)
        dp.register_callback_query_handler(self.menu_car_subtype, text="back",                                          state=self.registration_level3)

        dp.register_callback_query_handler(self.menu_cars, lambda x: x.data.startswith("truck"),                        state=self.registration_level2)
