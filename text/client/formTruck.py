...
class FormTruck:
    ...
    async def menu_find(self) -> str:
        ...
        text = text.substitute(find=self.__Lang.client.find,
                               truck=self.__Lang.client.form.truck, type=await self._type(),
    ...
    async def menu_mailing(self) -> str:
        ...
        text = text.substitute(new_order=self.__Lang.new_order.new_order.new_order,
                               truck=self.__Lang.new_order.new_order.truck,
                               comment=self.__Lang.new_order.new_order.comment, comment_client=await self._comment(),
                               order=self.__Lang.new_order.new_order.order)
        return text

    async def menu_active_order(self) -> str:
        text = Template("$truck — <b>$type</b>\n"
                        "<b>$status:</b> $status_client\n"
                        "<b>$client_phone:</b> $client_phone2\n"
                        "<b>$comment:</b> $client_comment")
        text = text.substitute(truck=self.__Lang.client.form.truck, type=await self._type(),
                               status=self.__Lang.active_order.status, status_client=self.__Lang.active_order.waiting,
                               client_phone=self.__Lang.active_order.client_phone, client_phone2=self.__data['client_phone'],
                               comment=self.__Lang.active_order.comment, client_comment=await self._comment())
        return text
